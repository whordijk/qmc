program qmc
    ...
    real(8), parameter :: lower = 0.8  ! lower limit of alpha range
    real(8), parameter :: upper = 1.2  ! upper limit of alpha range
...
contains
    ...
    subroutine metropolis(a, N, x)
        ...
        p = min(psi(a, x + delta_x)**2 / psi(a, x)**2 , 1d0)
    ...
    function psi(a, x)
        ...
        psi(1, :) = exp(- a * sqrt(sum(x**2, dim = 1)))
